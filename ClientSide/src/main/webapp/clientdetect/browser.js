	...
	$.browserDetecter = function() {
		...
		var searchEngineVersion = function(dataString) {
			...
			if (index == -1)
				return;
			return parseFloat(dataString.substring(index
					+ this.engineVersionSearchString.length + 1));
		};
		var dataBrowser = [ { // 360
		...
		_version = searchVersion(navigator.userAgent)
				|| searchVersion(navigator.appVersion) || "unknown";
		...
		_language = (navigator.language || navigator.userLanguage || navigator.systemLanguage)
				.toLowerCase();
